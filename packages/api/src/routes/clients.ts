...
const commPreference = z.enum(['sms', 'email', 'both', 'none']).default('email');
...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const result = await query('SELECT * FROM clients WHERE business_id = $1 AND active = true ORDER BY created_at DESC LIMIT $2 OFFSET $3', [getBusinessId(req), page.limit, page.offset], db(req));
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  const result = await query('SELECT * FROM clients WHERE id = $1 AND business_id = $2 AND active = true', [id, getBusinessId(req)], db(req));
...
router.patch('/:id', asyncHandler(async (req, res) => {
  ...
  const result = await query(`UPDATE clients SET ${patch.fields.join(', ')}, updated_at = now() WHERE id = $${patch.values.length + 1} AND business_id = $${patch.values.length + 2} AND active = true RETURNING *`, [...patch.values, id, getBusinessId(req)], db(req));
  if (!result.rows[0]) throw new AppError(404, 'Client not found', 'CLIENT_NOT_FOUND');
  res.json({ data: result.rows[0] });
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
  const { id } = idParamSchema.parse(req.params);
  await query('UPDATE clients SET active = false, updated_at = now() WHERE id = $1 AND business_id = $2', [id, getBusinessId(req)], db(req));
  res.status(204).send();
}));
