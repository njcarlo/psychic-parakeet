...
const createInvoiceSchema = z.object({ client_id: z.string().uuid(), job_ids: z.array(z.string().uuid()).min(1), due_at: z.coerce.date().optional(), currency: z.string().optional() });
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const invoice = await withBusinessContext(businessId, (client) => createInvoiceFromJobs(client, { businessId, clientId: body.client_id, jobIds: body.job_ids, dueAt: body.due_at, currency: body.currency }));
...
router.get('/:id', asyncHandler(async (req, res) => {
  ...
  const lines = await query('SELECT * FROM invoice_line_items WHERE invoice_id = $1 AND business_id = $2 ORDER BY sort_order', [id, getBusinessId(req)], db(req));
...
router.patch('/:id/sent', asyncHandler(async (req, res) => {
  const { id } = idParamSchema.parse(req.params);
  const result = await query("UPDATE invoices SET status = 'sent', issued_at = COALESCE(issued_at, now()), updated_at = now() WHERE id = $1 AND business_id = $2 AND status <> 'void' RETURNING *", [id, getBusinessId(req)], db(req));
...
router.patch('/:id/void', asyncHandler(async (req, res) => {
  const { id } = idParamSchema.parse(req.params);
  const result = await query("UPDATE invoices SET status = 'void', updated_at = now() WHERE id = $1 AND business_id = $2 RETURNING *", [id, getBusinessId(req)], db(req));
  if (!result.rows[0]) throw new AppError(404, 'Invoice not found', 'INVOICE_NOT_FOUND');
  res.json({ data: result.rows[0] });
}));
